...
foreach cc of local cnt {
	...
	*-----------------------------------------------------------------------------
			...
			save "$temp_dir\temp_`year'_PISA_v01_M_v01_A_CI_LEVELS_`cc'.dta", replace
		}
	}
	restore
}
...
foreach cc of local cnt {
	...
	append using "$temp_dir\temp_`year'_PISA_v01_M_v01_A_CI_LEVELS_`cc'.dta", replace
	...
	export excel using "$output\BRIEFS\`cc'.xlsx", sheetreplace sheet("Data_Levels_`ccc'") firstrow(variables)
}
